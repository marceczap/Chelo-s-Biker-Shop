...
<body class="min-h-screen flex flex-col items-center justify-center p-4">
  ...
  <div class="mb-3 text-center text-white max-w-lg">
    ...
    <div class="flex flex-wrap items-center justify-center gap-1.5 mt-2">
      ...
      <button onclick="openHamburgerMenu()" class="px-2.5 py-1 text-[11px] font-bold bg-blue-600/80 hover:bg-blue-600 rounded-full transition-all cursor-pointer">P7: Sucursales</button>
  ...
  <div class="phone-container select-none text-gray-900 p-0 flex flex-col justify-between relative overflow-hidden">
    ...
    <div id="screenLogin" class="flex flex-col items-center justify-between h-full w-full p-6 relative">
      ...
      <div class="w-full max-w-[280px] flex flex-col items-center space-y-2 mt-0.5 z-10">
        ...
        <!-- Main Action Button (INGRESAR en color verde) -->
        <div class="w-full flex justify-center mt-2">
          <button 
            type="button"
            onclick="handleLogin()" 
            class="w-full py-2.5 bg-emerald-600 hover:bg-emerald-700 active:scale-95 text-white font-serif font-black text-xs rounded-full tracking-widest transition-all shadow-md cursor-pointer"
          >
            INGRESAR
          </button>
        </div>
    ...
    <div id="screenHome" class="hidden flex flex-col h-full w-full relative overflow-y-auto bg-white">
      ...
      <div class="relative w-full h-52 rounded-b-[38px] overflow-hidden shadow-lg flex flex-col justify-between p-5 shrink-0 z-10">
        ...
        <!-- Top Header Navigation with HOME Centered, Profile and Hamburger Menu -->
    ...
    <div id="screenProfile" class="hidden flex flex-col h-full w-full relative bg-gray-50 overflow-hidden">
      ...
      <div class="h-16 bg-white border-t border-gray-100 flex items-center justify-around px-8 z-30 shadow-2xl shrink-0">
        ...
        <button onclick="openHamburgerMenu()" class="flex flex-col items-center justify-center p-2 text-gray-400 hover:text-black transition-all cursor-pointer">
...
\n    <!-- PANTALLA 4: CATÁLOGO DE MOTOS (DISPONIBLES) -->
    ...
    <div id="screenMotosCatalog" class="hidden flex flex-col h-full w-full relative bg-white overflow-hidden">
      ...
      <div class="h-16 bg-white border-t border-gray-100 flex items-center justify-around px-8 z-30 shadow-2xl shrink-0">
        <button onclick="goToProfile()" class="flex flex-col items-center justify-center p-2 text-gray-400 hover:text-black transition-all cursor-pointer">
          <svg class="w-7 h-7" fill="currentColor" viewBox="0 0 24 24"><path d="M12 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm0 2c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4z"/></svg>
        </button>
    ...
    <div id="screenBranchMap" class="hidden flex flex-col justify-between h-full w-full relative bg-white overflow-hidden">
      ...
      <div class="flex items-center justify-between px-5 pt-4 pb-2 z-10 bg-white/95 backdrop-blur-xs border-b border-gray-100 shrink-0">
        <button onclick="openHamburgerMenu()" class="text-gray-900 font-bold p-1 hover:opacity-70 active:scale-90 transition-all cursor-pointer">
          <svg class="w-6 h-6" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M15 19l-7-7 7-7"/></svg>
        </button>
  ...
  <script>
    ...
    // Multi-Product Add To Cart with Fly-to-Cart Animation
    function addProductToCartFromDetail() {{
      ...
      const srcImg = document.getElementById('detailImage');
      ...
      if (srcImg && cartBtn) {{
        ...
        flyClone.src = currentDetailItem.img;
        flyClone.className = 'fixed z-50 pointer-events-none rounded-xl object-contain shadow-2xl transition-all duration-500 ease-in-out';
        flyClone.style.width = '100px';
        flyClone.style.height = '100px';
        flyClone.style.left = (srcRect.left + (srcRect.width - 100) / 2) + 'px';
        flyClone.style.top = (srcRect.top + (srcRect.height - 100) / 2) + 'px';
        ...
        setTimeout(() => {{
          flyClone.style.left = (cartRect.left + (cartRect.width - 30) / 2) + 'px';
          flyClone.style.top = (cartRect.top + (cartRect.height - 30) / 2) + 'px';
          flyClone.style.width = '30px';
          flyClone.style.height = '30px';
          flyClone.style.transform = 'scale(0.3) rotate(360deg)';
      ...
      cart.forEach((item, index) => {{
        ...
        card.innerHTML = `
          ...
          <div class=\"flex flex-col items-center space-y-1 shrink-0\">
            <div class=\"flex items-center space-x-1.5 bg-gray-100 rounded-full p-1 border border-gray-200 shadow-inner\">
              <button onclick=\"changeCartItemQty(${{index}}, -1)\" class=\"w-7 h-7 rounded-full bg-gray-900 hover:bg-black text-white font-black text-sm flex items-center justify-center shadow-md active:scale-90 transition-all cursor-pointer\">−</button>
              <span class=\"text-xs font-black px-1.5 text-gray-900 min-w-[18px] text-center\">${{item.qty}}</span>
              <button onclick=\"changeCartItemQty(${{index}}, 1)\" class=\"w-7 h-7 rounded-full bg-gray-900 hover:bg-black text-white font-black text-sm flex items-center justify-center shadow-md active:scale-90 transition-all cursor-pointer\">+</button>
            </div>
